...
#include "esp_crt_bundle.h"   // NEW (Day 3 fix): needed for HTTPS server certificate verification
...
// =====================================================
#define WDT_TIMEOUT_MS 5000   // back to 5000 -- network_task is no longer watchdog-registered
                              // (see network_task for why), so the 8000ms headroom is not needed
...
#define MOCK_SERVER_URL "https://webhook.site/234ae639-a30e-490e-9caf-08d7b79b893c"
...
esp_err_t http_post_json(const char *json_str)
{
    esp_http_client_config_t config = {
        ...
        .crt_bundle_attach = esp_crt_bundle_attach,   // NEW (Day 3 fix): verifies the
                                                       // server's HTTPS certificate using
                                                       // ESP-IDF's built-in CA bundle --
                                                       // required for any https:// URL,
                                                       // e.g. webhook.site
    };
...
{
    // NOTE (Day 3 fix): NOT watchdog-registered. This task's natural cycle
    // is 10s (NETWORK_PERIOD_MS), which is longer than WDT_TIMEOUT_MS would
    // safely allow without weakening watchdog protection for the faster
    // sensor tasks. It's already protected against hanging by the
    // placeholder-URL skip check and HTTP_TIMEOUT_MS below.
    TickType_t last_wake = xTaskGetTickCount();
    int cycle = 0;
