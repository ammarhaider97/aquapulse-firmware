...
#define AGGREGATOR_PRIORITY 3   // more then a sensor tasks (priority 2), so queue will empty fastly
...
// manages which task gets CPU time and when.
    ...
    xTaskCreate(aggregator_task,"aggregator_task",AGGREGATOR_STACK,    NULL, AGGREGATOR_PRIORITY,  NULL);
