...
// =====================================================
// MOVED ABOVE the HTTP client section -- http_test_task() (below) uses
// wifi_event_group and WIFI_CONNECTED_BIT, so this must be defined first.
...
// =====================================================
// No mock endpoint URL was provided by the internship yet -- left as a
// clearly-marked placeholder. http_post_json() will simply fail with
// "[HTTP] POST failed: ..." until this is replaced with a real URL,
// which is expected and fine for now (http_test_task handles it
...
// =====================================================
// ===========  DAY 3 TEST TASK (temporary)  ===========
// =====================================================
#define HTTP_TEST_STACK 4096
...
static void http_test_task(void *pvParameters)
{
    while (1) {
        ...
        if (bits & WIFI_CONNECTED_BIT) {
            ...
            printf("[HTTP] Posting: %s\n", json);
            http_post_json(json);
            free(json);
        } else {
            printf("[HTTP] Skipped -- WiFi not connected yet\n");
        }
        vTaskDelay(pdMS_TO_TICKS(10000));
    }
}
...
void app_main(void)
{
    ...
    xTaskCreate(http_test_task, "http_test_task", HTTP_TEST_STACK, NULL, 1, NULL);
