...
// Sends one or more bytes to the OLED, along with a control byte
// (0x00 = command, 0x40 = pixel data) so the display knows how to
...
// ================  SSD1306 INIT  ====================
// =====================================================
// Standard SSD1306 init sequence from the datasheet: turns display off,
// sets clock/multiplex/charge-pump parameters, then turns display on.
...
// Sends the entire framebuffer to the OLED (only called when we want
// to refresh the screen — keeps I2C traffic low).
...
// ==============  MINIMAL 5x7 FONT  ==================
// =====================================================
// Each character is 5 bytes (5 columns), and each byte uses 7 bits
// (bit0 = topmost pixel, bit6 = bottommost pixel). Only the characters
// we actually need are defined here.
...
// Returns the glyph bitmap for a character. If the character isn't
// found in the table, it silently falls back to a blank space glyph
// (no error/crash — just renders nothing for that character).
...
// Draws one character at the given page (0-7) and column position.
...
// Draws a full string (1 pixel gap after each character).
void ssd1306_draw_string(int page, int col, const char *str)
{
    int x = col;
    while (*str) {
        ssd1306_draw_char(page, x, *str);
        x += 6; // 5 pixel char width + 1 pixel gap
        str++;
    }
}
...
// Linear interpolation between the nearest two anchor points.
// Returns the min/max NTU value if voltage is outside the calibrated range.
...
// Takes MEDIAN_WINDOW oversampled readings, sorts them, and returns
// the middle value — filters out sudden spikes/noise.
...
// =====================================================
// ============  CALIBRATION DATA + NVS (Day 3) =======
// =====================================================
// Holds calibration coefficients for all five sensors together, so we
// can save/load them as a single NVS blob instead of separate calls.
typedef struct {
    ...
    float ds18b20_offset_c;               // Temp sensor: calibration offset
} all_calibration_t;

#define NVS_NAMESPACE   "aqp_cal"
#define NVS_KEY_CALDATA "cal_blob"

// Factory-default values, used when nothing valid is found in NVS.
...
// Saves the calibration struct into NVS flash.
bool calibration_save_to_nvs(const all_calibration_t *cal)
{
    ...
    esp_err_t err = nvs_set_blob(handle, NVS_KEY_CALDATA, cal, sizeof(all_calibration_t));
    if (err == ESP_OK) err = nvs_commit(handle); // commit = actually write to flash
    nvs_close(handle);
    return err == ESP_OK;
}

// Reads the calibration struct back from NVS.
bool calibration_load_from_nvs(all_calibration_t *cal)
{
    nvs_handle_t handle;
    if (nvs_open(NVS_NAMESPACE, NVS_READONLY, &handle) != ESP_OK) return false;

    size_t required_size = sizeof(all_calibration_t);
    esp_err_t err = nvs_get_blob(handle, NVS_KEY_CALDATA, cal, &required_size);
    nvs_close(handle);
    return (err == ESP_OK && required_size == sizeof(all_calibration_t));
}
...
void app_main(void)
{
    // NVS must be initialized once at boot, before any flash access.
    ...
    // Load calibration from NVS at boot; fall back to factory defaults
    // if nothing has been saved yet.
    all_calibration_t calibration;
    if (!calibration_load_from_nvs(&calibration)) {
        printf("No saved calibration found -> loading factory defaults\n");
        calibration_load_defaults(&calibration);
        calibration_save_to_nvs(&calibration);
    } else {
        printf("Calibration loaded from NVS successfully\n");
    }
    ...
    while (1)
    {
        ...
        // Build and push the OLED text lines.
        char line[16];
        ssd1306_clear();
        ...
        // Button press: simulate a calibration change and save it to NVS.
        if (button_pressed) {
            calibration.ph_cal.slope += 0.1f;
            calibration_save_to_nvs(&calibration);
            printf("Button pressed -> calibration updated & saved (slope=%.2f)\n", calibration.ph_cal.slope);
            button_pressed = 0;
        }
