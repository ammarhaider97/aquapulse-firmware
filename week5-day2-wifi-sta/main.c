...
#include "cJSON.h"   // NEW (Week 5 - Day 1): JSON payload building
#include "esp_wifi.h"
#include "esp_event.h"
#include "esp_netif.h"
#include "esp_timer.h"
...
// =====================================================
// NEW: this global struct is what aggregator_task writes into (under
// mutex) and what the network/JSON code reads from (under the same
// mutex) to always get the LATEST full snapshot of all 5 sensors.
typedef struct {
    float ph;
    float tds_ppm;
    float turbidity_ntu;
...
static SemaphoreHandle_t readings_mutex;   // NEW (Week 5 - Day 1)
...
static void aggregator_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        // NEW (Week 5 - Day 1): mirror the local latest[] array into the
        // shared g_readings struct so other tasks (JSON builder) can see it.
...
// =====================================================
// Each bit = one sensor being physically out of its valid range.
// 0 = everything looks sane. Multiple bits can be set together.
...
// Physically-plausible bounds. Anything outside means the reading is
// not trustworthy (sensor fault, bad wiring, ADC saturation, etc.)
#define PH_MIN         0.0f
#define PH_MAX         14.0f
#define TDS_MIN        0.0f
...
// Builds the final JSON string matching the backend schema:
// { device_id, firmware_version, timestamp, readings: {...}, fault_flags }
// Any field outside its valid range is replaced with 0.0 and its bit is
// set in fault_flags -- this is the "reject before serialisation" step.
...
char *build_json_payload(const system_readings_t *r, uint32_t *fault_flags_out)
{
    ...
    // Uptime in seconds used as timestamp for now -- real wall-clock time
    // (SNTP) is out of scope for this week; backend can stamp receive-time.
    ...
    cJSON_AddItemToObject(root, "readings", readings);   // ownership moves to root

    cJSON_AddNumberToObject(root, "fault_flags", fault_flags);

    char *json_str = cJSON_PrintUnformatted(root);
    cJSON_Delete(root);   // safe -- PrintUnformatted already copied the string out
...
// =====================================================
#define WIFI_SSID  "Wokwi-GUEST"   // Wokwi's built-in open network
#define WIFI_PASS  ""              // no password on Wokwi-GUEST
                                ...
                                int32_t event_id, void *event_data)
{
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        ...
        // Non-blocking: schedule a one-shot timer instead of vTaskDelay here,
        // so the event-loop task stays free to process other events.
        esp_timer_start_once(reconnect_timer, (uint64_t)s_retry_delay_ms * 1000); // us
        ...
        s_retry_delay_ms = WIFI_RETRY_MIN_MS;   // reset backoff after success
...
// =====================================================
// ===========  DAY 1 TEST TASK (temporary)  ===========
// =====================================================
// Exists only for Day 1 testing -- will be replaced by network_task on
// Day 4. Reads the shared snapshot, builds JSON, prints it every 10s so
// we can visually confirm the payload before adding WiFi/HTTP.
#define JSON_TEST_STACK 4096

static void json_test_task(void *pvParameters)
{
    int cycle = 0;
    while (1) {
        ...
        // Fault injection demo: every 3rd cycle, corrupt a LOCAL COPY only
        // (never touches g_readings / real sensor state) to prove the
        // validator catches it.
        cycle++;
        if (cycle % 3 == 0) {
            printf("[TEST] Injecting out-of-range pH (20.0) this cycle...\n");
            snapshot.ph = 20.0f;
        }

        uint32_t flags;
        char *json = build_json_payload(&snapshot, &flags);
        printf("[JSON] %s\n", json);
        if (flags != 0) {
            printf("[JSON] fault_flags = 0x%02lX -- one or more fields were rejected & zeroed\n",
                   (unsigned long)flags);
        }
        free(json);   // cJSON_PrintUnformatted uses malloc internally
...
void app_main(void)
{
    ...
    readings_mutex = xSemaphoreCreateMutex();   // NEW (Week 5 - Day 1)
    ...
    // NEW (Week 5 - Day 1): temporary test task for the JSON payload builder.
    xTaskCreate(json_test_task, "json_test_task", JSON_TEST_STACK, NULL, 1, NULL);
