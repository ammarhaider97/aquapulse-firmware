...
// =====================================================
// A queue acts like a "mailbox" that safely transfers data between two tasks
// (FreeRTOS automatically prevents race conditions when using queues,
// so a mutex isn't required here). Each sensor task places its reading into
// a struct and "posts" it to the queue (xQueueSend). On the other end, a single
// "aggregator" task waits to pull items out of the queue (xQueueReceive),
// combining them to display on the OLED + serial interface.
//
// We are using just ONE shared queue where every reading includes
// its corresponding sensor_id tag — creating 5 separate queues was also an option,
// but a single queue is simpler and sufficient for this scale (5 tasks).
...
// =====================================================
// This task is the only one that retrieves readings from the queue. Whenever a
// new reading arrives, we update that sensor's value in our local "latest[]" array,
// then output the complete combined snapshot to the OLED and serial log. This way,
// the OLED (which is a shared resource) is accessed only by this single task —
// eliminating the need for a mutex here.
#define AGGREGATOR_STACK    3072
#define AGGREGATOR_PRIORITY 3  // slightly higher than sensor tasks (priority 2), so the queue empties quickly
...
// =====================================================
// A watchdog is a safety timer: if a monitored task fails to send a
// "I am alive" signal (esp_task_wdt_reset) within X seconds,
// the watchdog assumes the task has hung or deadlocked
// and force-reboots the board (a crash is better than staying
// frozen indefinitely). We are only guarding the AGGREGATOR task
// because it is the central point — if it hangs, the entire system's
// display and logging will freeze. Since the aggregator loops every ~1s,
// a 5s timeout provides plenty of buffer.
#define WDT_TIMEOUT_MS 5000  // aggregator loops every ~1s, 5s is plenty of buffer
...
void app_main(void)
{
    ...
    // app_main is also a FreeRTOS task itself (with priority slightly above idle).
    // We keep it alive to check for button presses, just like in
    // Week 3 — it will continue to run this way in Day 2+ as well.
